...
userApp.post('/sendemail', async(req, res) => {
    ...
    try{
        ...
        res.status(200).json({success:true, message:'Email Sent', otpToken: token, otp: hashedOtp})
    }catch(err){
...
userApp.post('/update-password', expressAsyncHandler( async(req, res) => {
    ...
    if(req.body.newPass !== req.body.repeatPass){
        res.send({message: '*New Password and Repeat Password must be Same..!!'})
        return
    }
    ...
    await usersCollectionObj.updateOne({userid: user}, {$set: {password: hashedPassword}})
